...
class MessageGrouper:
    ...
    def _pk_to_nested_and_composite_field(self, field: Optional[Union[str, List[str], List[List[str]]]]) -> List[List[str]]:
        if not field:
            return [[]]
        return [[field]] if isinstance(field, str) else [[i] for i in field] if isinstance(field[0], str) else field
    ...
    def get_message_groups(
        ...
        slices, log_messages, auxiliary_requests, latest_config_update = [], [], [], None
        for message_group in self._get_message_groups(
            self._read_stream(source, config, configured_catalog, state),
            schema_inferrer,
            datetime_format_inferrer,
            ...
            if isinstance(message_group, AirbyteLogMessage):
                ...
                log_messages.append(LogMessage(message=f"{message_group.error.message} - {message_group.error.stack_trace}", level="ERROR"))
            elif isinstance(message_group, AirbyteControlMessage):
                if not latest_config_update or latest_config_update.emitted_at <= message_group.emitted_at:
                    latest_config_update = message_group
            elif isinstance(message_group, AuxiliaryRequest):
    ...
    def _get_message_groups(
        ...
        records_count = 0
        at_least_one_page_in_group = False
        current_page_records, current_slice_pages = [], []
        current_page_request, current_page_response, current_slice_descriptor, latest_state_message = None, None, None, None
        ...
        while records_count < limit and (message := next(messages, None)):
            ...
            if self._need_to_close_page(at_least_one_page_in_group, message, json_object):
                self._close_page(current_page_request, current_page_response, current_slice_pages, current_page_records)
            ...
            if (
                ...
                if json_object is not None and self._is_http_log(json_object):
                    if self._is_auxiliary_http_request(json_object):
                        airbyte_cdk = json_object.get("airbyte_cdk", {})
                        if not isinstance(airbyte_cdk, dict):
                            raise ValueError(f"Expected airbyte_cdk to be a dict, got {airbyte_cdk} of type {type(airbyte_cdk)}")
                        stream = airbyte_cdk.get("stream", {})
                        if not isinstance(stream, dict):
                            raise ValueError(f"Expected stream to be a dict, got {stream} of type {type(stream)}")
                        title_prefix = "Parent stream: " if stream.get("is_substream", False) else ""
                        http = json_object.get("http", {})
                        if not isinstance(http, dict):
                            raise ValueError(f"Expected http to be a dict, got {http} of type {type(http)}")
                        yield AuxiliaryRequest(
                            title=title_prefix + str(http.get("title", None)),
                            description=str(http.get("description", None)),
                            request=self._create_request_from_log_message(json_object),
                            response=self._create_response_from_log_message(json_object),
                        )
                    else:
                        at_least_one_page_in_group = True
                        current_page_request = self._create_request_from_log_message(json_object)
                        current_page_response = self._create_response_from_log_message(json_object)
                else:
                    yield message.log
    ...
    def _has_reached_limit(self, slices: List[StreamReadSlices]) -> bool:
        ...
        return any(len(_slice.pages) >= self._max_pages_per_slice or record_count >= self._max_record_limit for _slice in slices)
    ...
    @staticmethod
    def _clean_config(config: Dict[str, Any]) -> Dict[str, Any]:
        return {k: v for k, v in deepcopy(config).items() if not k.startswith("__")}
